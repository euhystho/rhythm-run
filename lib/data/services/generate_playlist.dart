...
Future<Playlist> getSimilarSongs(Song song) async {
  const String apiKey = "5e89f5c28e4a9e2e661576f3ab8bddf3";
  const String sharedSecret = String.fromEnvironment('LASTFM_SHARED_SECRET');
  ...
  try {
    ...
    var similarSongs = await lastfm.read(
      'track.getSimilar',
      {"artist": song.artist, "track": song.name, "limit": "10"}
    );
    ...
    var tracks = trackElements.map((element) {
      try {
        ...
        final artistName = artistElement.findElements('name').first.innerText;
        
        // Extract the match score and mbid:
        final match = double.tryParse(element.findElements('match').first.innerText) ?? 0.0;
...
void main() async {
  ...
  for (var song in playlist.tracks) {
    if(song is SimilarSong){
      print("Song: ${song.name}, Artist: ${song.artist} - Match: ${song.match}");
    }
